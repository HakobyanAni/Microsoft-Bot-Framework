...
- Setup LUIS
    ...
    - As the command `msbot clone services` uses secret-manager to safely store the key generated for encrypting the bot file, it's necessary adding the next code in your CoreBot.csproj file:
    ...
    **Note**: You can add any arbitrary `GUID` value for the `UserSecretsId` property, but you can not repeat `GUID` values among the projects, as they must be unique.
    ...
    msbot clone services --name "CoreBot" --luisAuthoringKey <LUIS-authoring-key> --code-dir "." --location westus --sdkLanguage "CSharp" --folder deploymentScripts/msbotClone --verbose
    ```

    **Note**: Once the Luis service application is created, change the value of `LuisConfiguration` variable in CoreBotBot.cs with the Luis service name, which can be found in CoreBot.bot file (it should be `CoreBot_core-bot-LUIS`).
...
msbot clone services --name "CoreBot" --code-dir "." --location westus --sdkLanguage "Csharp" --folder deploymentScripts/msbotClone --verbose
```
